      ...
      fetch('https://api.brevo.com/v3/smtp/email', {
        ...
        body: JSON.stringify({
          ...
          htmlContent: (() => {
            ...
            // Dark mode logo on the gradient header (visible on dark bg)
            const logoDarkHtml = siteUrl ? `<img src="${siteUrl}/images/logo-modeSombre.png" alt="JUDDEV" style="height:56px;margin-bottom:0.75rem;object-fit:contain;display:block;margin-left:auto;margin-right:auto" />` : '';
            // Light mode logo as profile circle avatar
            const logoProfileHtml = siteUrl ? `<img src="${siteUrl}/images/logo-modeClair.png" alt="JUDDEV" style="width:56px;height:56px;border-radius:50%;object-fit:contain;background:#fff;border:2px solid rgba(255,255,255,0.3);padding:4px" />` : `<div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#0066ff,#00d4ff);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:1.4rem;color:#fff">J</div>`;
            return `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0a0e1a;color:#e2e8f0;border-radius:12px;overflow:hidden"><div style="background:linear-gradient(135deg,#0066ff,#00d4ff);padding:2rem;text-align:center">${logoDarkHtml}<h1 style="color:#fff;margin:0;font-size:1.4rem">JUDDEV CORPORATION</h1><p style="color:rgba(255,255,255,0.85);margin:0.5rem 0 0">Nouveau message de contact</p></div><div style="padding:2rem"><div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;padding:1rem;background:rgba(255,255,255,0.04);border-radius:8px;border:1px solid rgba(255,255,255,0.08)">${logoProfileHtml}<div><div style="font-weight:700;color:#fff;font-size:1rem">${name}</div><div style="color:#94a3b8;font-size:0.875rem">${email}</div></div></div><p><strong style="color:#94a3b8">Téléphone:</strong> <span style="color:#e2e8f0">${phone || 'N/A'}</span></p><p><strong style="color:#94a3b8">Sujet:</strong> <span style="color:#e2e8f0">${subject || 'N/A'}</span></p><p><strong style="color:#94a3b8">Message:</strong></p><p style="white-space:pre-wrap;color:#e2e8f0;background:rgba(255,255,255,0.05);padding:1rem;border-radius:8px;border-left:3px solid #0066ff">${message}</p></div><div style="padding:1rem 2rem;background:rgba(0,0,0,0.2);text-align:center"><p style="color:#64748b;font-size:0.8rem;margin:0">© 2026 JUDDEV CORPORATION — Yaoundé, Cameroun</p></div></div>`;
          })()
        })
      }).then(r => {
